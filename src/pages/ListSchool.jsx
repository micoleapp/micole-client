...
function ListSchool() {
  ...
  const [value2, setValue2] = React.useState(
    precios.length > 0 ? precios[1] : 5000
  );
  ...
  const goToDetails = (id, lista) => {
    if (gradoName.length === 0 || ingresoName.length === 0) {
      ...
      window.open(
        `/#/schooldetail/${id}?grado=${gradoName}&ingreso=${ingresoName}&lista=${lista}`,
        "_blank"
      );
    }
  };
            ...
            >
              ...
              <div>
                <div className="flex items-center gap-5 z-50 ">
                  ...
                  <button
                    onClick={() => setToggleDificultad(!toggleDificultad)}
                  >
                    {" "}
                    <FontAwesomeIcon
                      size="lg"
                ...
                >
                  {types.map((type, index) => (
                    <MenuItem
                      ...
                      key={index}
                      onClick={() =>
                        type.value !== "" && handleSort(type.value)
                      }
                    >
            ...
            <div className="bg-white w-fit flex px-2 py-1 rounded-md border">
              <form className="flex" onSubmit={(e) => handleSearch(e)}>
                <input
                  ...
                  name="search"
                  id="search"
                  className="outline-none w-[300px] lg:w-[400px]"
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      handleSearch(e);
                    }
